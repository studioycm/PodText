...
- Run only tests affected by recent changes (Tia): `./vendor/bin/pest --tia`.
  Do not add `--parallel` — see the note under Tia below.
...
> **PodText:** upstream documents this with `--parallel`. Do not use that flag
> here. The machine-global lane lock taken in bootstrap by `tests/Pest.php`
> refuses paratest's workers, so such a run executes ZERO tests and exits
> non-zero. It is a lock property, not a cost property, and is unrelated to the
> Xdebug instrumentation tax measured on this suite. Pinned by
> `tests/Feature/InstalledSkillGuardTest.php`.

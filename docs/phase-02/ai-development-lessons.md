...
- Prompt files must carry an explicit version line. If a kickoff names a prompt version and the checked-out prompt differs, stop before implementation and ask for direction. Requirements from a later prompt version that was not the executed contract should be carried into a future prompt's Job 0 instead of being silently lost or retroactively applied to the earlier run.

...
| Public Front v2 Step 10R-IP2 episode page header/info layout | Complete | this commit: `feat: refine episode podcast identity settings` | Extends `item_page` with `show_breadcrumbs`, `podcast_identity`, and ordered `info_fields`; adds settings migrations and admin controls; rebuilds the public episode header with item/podcast image fallback, linked podcast identity, configured date labels/icons, linked category/tag/transcriber info fields, and site-wide link-audit tests. Review-fix coverage adds podcast identity style (`badge`, `text`, `title`, `hidden`), size, semantic/image-sampled color tokens, and placement above/below/before/after the title. R1 page part and R11-R18 are landed. |
| Public Front v2 Step 10R-IP3 transcript actions and reading UX | Complete | this commit: `feat: add transcript reading controls and actions menu` | Adds `item_page.show_transcript_actions_menu` default false, settings migration/admin toggle, share block above the player, transcript details row, settings-gated Blade/Alpine actions menu, font-size controls, fullscreen reading mode, and player/media column toggle. R7-R10 and R19-R22 are landed. M6 owns full R1-R23 verification and C1 final status next. |
...
- The active post-B3 implementation sequence is Step 10R-M6 before Step 10R-P1 through Step 10R-P3, then Step 10R-B4 can resume. Urgent Step 10R-HF1, Step 10R-M5, Step 10R-IP1, Step 10R-IP2, and Step 10R-IP3 are complete; IP2 includes the podcast identity style/position/image-color review fix, and IP3 includes local-only transcript reading controls. The original Step 10R-C1 single-author attribution task must not run as written. Step 9F/10F Footer + Rich Section Builder must wait until M1-M6, IP1-IP3, P1-P3, B4, and C2 are complete. Step 11 Seeders, Demo Data, Assets, and Cleanup must wait for approved Step 10R and Step 9F/10F completion or explicit Yoni approval. Prompt 13 has not started. The full Step 2 transcription publication workflow remains deferred/reserved; M3/M4 only added the minimal public read/display/count policy.
...
- Prompt 13 dashboard metrics remains not started. Public Front v2 post-Step-10 mini-step sequencing is controlled by `docs/phase-02/public-front-v2-step10r-9f-mini-step-ledger.md`; Step 10R-IP2 is complete in this commit including the podcast identity review fix, and the next mini-step is Step 10R-IP3.
...
- Future Step 9F/10F Footer + Rich Section Builder foundation remains planned after Step 10R-M1 through Step 10R-M6, Step 10R-B4, and Step 10R-C2 are complete; the active next mini-step after Step 10R-M3 is Step 10R-M4.

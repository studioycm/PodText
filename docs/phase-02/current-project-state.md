...
| FIX1 - Fetcher direct import and workspace publishing fixes | Complete | Pending implementation commit hash; `docs/phase-02/fetcher-workspace-fix1-handoff.md` | Makes fetcher CSVs importable by strict native importers with stable keys, adds direct import create/link/skip flow, improves workspace Spotify fetch options, applies publication-date autofill, upgrades trusted raw HTML editors/rendering, adds settings cache rider coverage, and enforces HTTP fixture discipline. No Composer/npm changes. |

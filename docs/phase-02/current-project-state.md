...
- AUTHZ1 is an approved prerequisite decision, not an implemented feature. Before ARCH1, PodText will replace the fixed enum/rank authorization authority with Spatie Laravel Permission plus Filament Shield, while retaining an owned ability catalog for stable keys, groups, bilingual labels, default role grants, and policies. Role bundles plus optional direct grants are approved; explicit denials are not. Super-admin bypass remains subject to last-super-admin/self-demotion, validation, conflict, reference, retention, and confirmation safeguards. Meaningful mini-abilities are grouped for management rather than generated for every UI click. Neither package is installed today; the existing `users.role` column, rank helpers, gates/macros, Users Resource, command, assignments, and tests remain current authority until a lossless all-panel migration is accepted. Research: `docs/research/settings-performance/09-arch1-drafts-authorization-research.md`. Remaining operator questions are indexed in `docs/research/settings-performance/10-pending-decision-question-queue.md`; Groups 1–15 are settled and Group 16 is next.
- ARCH1 is an approved forward architecture decision, not an implemented feature. After AUTHZ1 and before SP3D, Card Templates and Public Forms must migrate from `PublicContentSettings` into independently managed, versioned model/Resource aggregates with per-user mutable autosave working drafts, explicit immutable revision checkpoints, stable identities, published pointers, exact submission revision binding, protected revision non-hydration, data-preserving cutover/rollback, and one shared after-commit configuration-change coordinator. Autosave never overwrites another user's draft: only its owner mutates it; authorized users may read or adopt by copying into their own row. Explicit checkpoints join shared attributed history. Stale-base work may checkpoint as divergent but cannot publish before explicit comparison/rebase/acceptance. An authorized publisher may publish another author's immutable checkpoint, never their mutable row. Inactive working drafts are hidden as stale after 90 days but never automatically deleted. Bounded Public Forms policy such as `require_email_verification` remains typed settings. Unreferenced immutable drafts retain at least 90 days and the latest 20; cloning copies one selected revision into a new unprotected identity. After accepted ARCH1 migration, SP3D deletes legacy Template/Form settings branches and the surviving schema monolith, repairs/proves all three browser harnesses, and calibrates durable budgets on the final Resources and surviving settings pages. SP4 then applies slice reads/server-derived change sets to surviving singleton settings; LOG1 follows. Controlling records: `docs/research/settings-performance/07-sp3d-pre-research.md`, `09-arch1-drafts-authorization-research.md`, and the restart queue `10-pending-decision-question-queue.md`.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Decisions approved; implementation not started | `docs/research/settings-performance/09-arch1-drafts-authorization-research.md` | Selected Shield + Spatie Permission + owned PodText ability catalog. Requires lossless migration of the current fixed roles and all existing authorization surfaces before ARCH1. No dependency/schema/code change, implementation plan, or prompt exists yet. |
| ARCH1 - Versioned Card Template and Public Form aggregates | Decisions approved; implementation not started | `docs/research/settings-performance/07-sp3d-pre-research.md` | Supersedes the settings-only forward architecture for Templates/Forms. After AUTHZ1, ARCH1 research, implementation, no-loss migration, and operator acceptance precede SP3D. Current SP3C/Step 6 code remains authoritative migration-source and rollback evidence until cutover. No implementation plan or prompt has been approved. |

...
- Episode workspace EP1-R docs-only research and planning are recorded in `docs/research/episode-workspace/00-ep1-research.md` and `docs/phase-02/episode-workspace-plan.md`; EP1 implementation has not started and is waiting for Yoni review.
...
| IMG-A image naming foundation and Curator media library | Complete / awaiting final commit | `docs/research/images-media/01-imga-curator-research.md`; `docs/phase-02/images-arc-imga-implementation-plan.md`; `docs/phase-02/images-media-track-plan.md` | Implemented the one-run IMG-1 + IMG-2 merge: slug-default `AdminUxSettings::media_naming_strategy`, Curator v5 admin media library, cover alt text, app-owned Curator/FileUpload picker factory that stores path strings, app-owned cover cleanup, and in-place legacy cover/settings asset registration. Targeted IMG-A tests pass. The single full `php artisan test` run failed before three integration fixes; those exact failures now pass targeted and the full suite was not rerun per the IMG-A test policy. Pint, FilaCheck, build, and `git diff --check` pass. |

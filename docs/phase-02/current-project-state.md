...
| Step 10R-HF3 imports-exports queue and export row loading hotfix | Complete | `7d80c99` plus `fix: complete imports-exports hotfix across exporters with shared lifecycle tracing` | Adopted the manual queue fix retroactively and completed the bug class. The `imports-exports` queue was absent from Horizon supervisor config and waits, so Filament imports/exports sat unconsumed in Redis. Export row processing then lazy-loaded relation-derived exporter columns under non-production `Model::preventLazyLoading()` and failed all rows locally. The tracer and `import_export` daily channel are now in place; all five exporters share queue/batch/tag/notification lifecycle behavior; ContentGroup, ContentItem, Category, and Transcription exporters now eager-load every relation their export columns read; AuthorExporter has no relation-derived columns. |

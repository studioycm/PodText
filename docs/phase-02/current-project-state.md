...
- AUTHZ1-C is implemented from the accepted v1 contract, its canonical final gate is green, and its two-commit closeout is complete: `0147ea83947ccedb1336a71d8eecc887eb8d4e07` followed by hash stamp `628429236c138ad51fcb4b6d8311ad4726afb439`. It adds the fail-closed privacy-safe raw analyzer, exact accepted-fingerprint transactional five-role projection, immutable prepared/cache/complete evidence and receipt, post-commit cache recovery, true no-op reconciliation, and receipt-scoped pre-cutover rollback. Legacy `users.role`, ranks, Gates/macros, panel/Horizon/maintenance admission, User Resource restrictions, and every caller remain authoritative; `User` still lacks `HasRoles`. Handoff: `docs/phase-02/authz1c-analyzer-backfill-handoff.md`. No production/development-database action, authority cutover, compatibility grant, management UI, AUTHZ1-D–I, ARCH1, SP3D, or MAINT-LW-UX1 work began.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Foundation and AUTHZ1-C implementation/two-commit closeout complete with green final gate; authority cutover not started | Foundation handoff; `docs/research/settings-performance/12-authz1-pre-implementation-research.md`; `13-authz1-foundation-research.md`; `15-authz1c-analyzer-backfill-*`; `docs/phase-02/authz1c-analyzer-backfill-handoff.md` | The exact reversible foundation is shipped. C adds raw fail-closed analysis, exact accepted-fingerprint role projection, post-commit cache/crash recovery, idempotency, parity, and receipt rollback. Legacy enum/rank/Gates remain authoritative; D–I remain unstarted and the MySQL rehearsal remains a separate future production gate. |

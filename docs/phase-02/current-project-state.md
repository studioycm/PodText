...
- SP3A settings foundations are complete as `88fdda2 perf: add settings measurement protocol, lock surface, and import overlay`: a deterministic approximately 37 KB measurement mode and decomposed response/query headers; request-scoped, group-and-payload-keyed lifecycle memoization with byte-identical units; section/important-field lock surfaces with retired-lock reporting; acting-user overlays on import/restore/merge plus anonymous protection during normalization; and a full admin Select loading sweep. `AdminUxSettings` remains outside lifecycle import/locks. Handoff: `docs/phase-02/settings-sp3a-handoff.md`; research and plan are under `docs/research/settings-performance/03-sp3a-*`.
...
- SP3B settings subject pages and fresh owned-path saves are complete in the current run: eight ordinary focused pages, temporary Card Templates, and Manage Public Forms now preserve disjoint stale-page changes through a fresh canonical settings snapshot and registry-owned root overlay; legacy monolith URLs redirect; lifecycle/import/restore/normalize/lock/backup/Admin UX paths and the SP3A SHA remain unchanged. Handoff: `docs/phase-02/settings-sp3b-handoff.md`; research and plan are under `docs/research/settings-performance/04-sp3b-*`. Browser measurement sampling is deferred because the local browser runtime failed before navigation; no samples were fabricated.
- SP3C template library and one-template editor are complete in the current run: the temporary writable whole-list Card Templates page is now an unpaginated read-only Filament library backed by one fresh settings snapshot and one projected HomepageSection reference scan; separate create/edit pages mount one authorized draft; the focused writer provides sequential stale detection, reference/default/collision guards, fresh protected-part restoration, strict sibling/foreign-root preservation, and the existing one-save lifecycle. The production-shaped preview canary passed its 70% stop/go threshold and froze +20% budgets before final adoption. Browser DOM/heap/listener/back-navigation sampling remains an explicit pending operator check; no sample was fabricated. Handoff: `docs/phase-02/settings-sp3c-handoff.md`; research and plan are under `docs/research/settings-performance/05-sp3c-*`.
- AUTHZ1's reversible package/schema/catalog foundation is complete. Exact Shield 4.2.0 and Permission 7.3.0 are installed with transitive-only Plugin Essentials 1.2.1; Shield is configured but deliberately unregistered because installed 4.2.0 always exposes its Role Resource when registered. The independently frozen application catalog contains 135 literal abilities with canonical SHA-256 `fb46f5ef0228c2017e049b13a6f18eb72183a85b89249385828bf5295b9193c7`, complete HE/EN metadata, exact five-role metadata, and declarative Super 135/Admin 89/46/three-empty compatibility grants. The package migration is proven reversible by isolated SQLite `up/down/up`. The foundation itself introduced no package assignments, `HasRoles`, sync, policy/Gate cutover, management UI, direct grants, production action, or development-database action; AUTHZ1-C now owns the later controlled role/pivot projection described below. The operator-authorized maintenance correction terminates denied stale public Livewire updates with the existing exact 503 response while preserving Super/Admin bypass and all initial HTTP/form behavior. `users.role`, ranks, current Gates/macros, Users Resource behavior, Horizon, and callers remain authoritative. Handoff: `docs/phase-02/authz1-foundation-handoff.md`; research/plan: `docs/research/settings-performance/13-authz1-foundation-*`.
- The maintenance Livewire enforcement effects audit v1 is complete in `docs/research/settings-performance/14-maintenance-livewire-enforcement-effects-audit.md`. Its server correction is accepted. The medium production stale-tab notification/body/retry UX and focused missing regression coverage are named `MAINT-LW-UX1`, deferred independently, and due before the first later public Livewire navigation/polling/lazy/deferred/stream/upload expansion or AUTHZ1 final acceptance, whichever occurs first.
- AUTHZ1-C implementation and its two-commit closeout remain complete at `0147ea83947ccedb1336a71d8eecc887eb8d4e07` plus hash stamp `628429236c138ad51fcb4b6d8311ad4726afb439`, but the independent `AUTHZ1-C-AUDIT-01` audit found one High, four Medium, and two Low gaps in physical rollback ownership, real absent-cache completion, strict exactly-once cache evidence, rollback crash recovery, runtime schema-contract completeness, privacy-key validation, and direct test coverage. Decision 2 controls. The separate remediation is now researched, implementation-planned, and prompted at `docs/research/settings-performance/17-authz1c-audit-remediation-research.md`, `docs/research/settings-performance/17-authz1c-audit-remediation-implementation-plan.md`, and `prompts/pre-13-prompts/authz1c-audit-remediation-codex-prompt.md` v1; it is not implemented. The contract freezes physical receipt evidence, explicit non-rollback-capable unowned completion, truthful at-least-once idempotent cache invalidation, rollback prepared/recovery evidence, complete runtime schema validation, Laravel-compatible key validation, and deep artifact tests. Legacy `users.role`, ranks, Gates/macros, panel/Horizon/maintenance admission, User Resource restrictions, and every caller remain authoritative; `User` still lacks `HasRoles`. AUTHZ1-D–I remain blocked. The disposable MySQL rehearsal remains a separate future gate. No production/development-database action, authority cutover, compatibility grant, management UI, AUTHZ1-D–I, ARCH1, SP3D, or MAINT-LW-UX1 work began.
- ARCH1 is an approved forward architecture decision, not an implemented feature. After AUTHZ1 and before SP3D, Card Templates and Public Forms must migrate from `PublicContentSettings` into independently managed, versioned model/Resource aggregates with per-user mutable autosave working drafts, explicit immutable revision checkpoints, stable parent/revision/node identities, published pointers, exact submission revision binding, protected revision non-hydration, data-preserving cutover/rollback, and one shared after-commit configuration-change coordinator. Autosave never overwrites another user's draft: only its owner mutates it; authorized users may read or adopt by copying into their own row. Explicit checkpoints join shared attributed history. Stale-base work may checkpoint as divergent but cannot publish before explicit comparison/rebase/acceptance. An authorized publisher may publish another author's immutable checkpoint, never their mutable row. Inactive working drafts are hidden as stale after 90 days but never automatically deleted. BQ3 requires exactly one active published default per registered runtime Template family. BQ4 adds separate submission-PII view/export abilities and no routine hard-delete/prune before an approved privacy/retention policy. BQ5 makes raw-source AUTHZ1/ARCH1 cutover fail closed without fabricated identities/provenance. Bounded Public Forms policy such as `require_email_verification` remains typed settings. Unreferenced immutable drafts retain at least 90 days and the latest 20; cloning copies one selected revision into a new unprotected identity. Initial Form mount authorization is config-backed for two hours, separate from five-minute OTP. After accepted ARCH1 migration and post-ownership L10N/ADM/LENS work, SP3D deletes legacy Template/Form settings branches and the surviving schema monolith, repairs/proves all three browser harnesses, and calibrates durable budgets on final Resources/surviving settings pages. SP4 then applies slice reads/server-derived change sets; LOG1 follows. Controlling records: `docs/research/settings-performance/07-sp3d-pre-research.md`, `09-arch1-drafts-authorization-research.md`, `10-pending-decision-question-queue.md`, and `11-bq-decisions-and-defaults-audit.md`.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Foundation/C implementation complete; C remediation planned and prompted but not implemented; authority cutover not started | Foundation/C/audit handoffs; `15-authz1c-analyzer-backfill-*`; `16-authz1c-independent-analyzer-backfill-audit.md`; `17-authz1c-audit-remediation-*`; `prompts/pre-13-prompts/authz1c-audit-remediation-codex-prompt.md` v1 | Audit decision 2's R-01–R-05 remediation is implementation-ready but unexecuted. Legacy enum/rank/Gates remain authoritative; D–I remain blocked and the MySQL rehearsal remains a separate future gate. |
| MAINT-LW-UX1 - Stale-tab maintenance UX and regressions | Named deferred; not implemented or prompted | `docs/research/settings-performance/14-maintenance-livewire-enforcement-effects-audit.md` | Independent medium UX/test follow-up due before the first later public Livewire navigation/polling/lazy/deferred/stream/upload expansion or AUTHZ1 final acceptance, whichever comes first; not an AUTHZ1-C blocker. |
| ARCH1 - Versioned Card Template and Public Form aggregates | Decisions approved through BQ3–BQ6; implementation not started | `docs/research/settings-performance/07-sp3d-pre-research.md`; `11-bq-decisions-and-defaults-audit.md` | Supersedes settings-only Templates/Forms. Requires exactly one published default per runtime Template family, interim PII deletion freeze, raw-source fail-closed cutover, and separately accepted boundaries before SP3D. Current SP3C/Step 6 code remains migration-source/rollback evidence. No implementation plan or prompt has been approved. |

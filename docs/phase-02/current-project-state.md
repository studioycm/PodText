...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Foundation complete; authority cutover not started | `docs/phase-02/authz1-foundation-handoff.md`; `docs/research/settings-performance/12-authz1-pre-implementation-research.md`; `13-authz1-foundation-research.md` | Installs the exact reversible package/schema foundation; adds the immutable 135-ability bilingual catalog, five-role metadata, declarative compatibility manifest, dormant Shield boundary, production mutation-command guard, and exact five-role no-regression matrix. The narrow maintenance/Livewire correction preserves the declared bypass audience. Legacy enum/rank/Gates remain authoritative; AUTHZ1-C analyzer/backfill requires a new accepted slice. |

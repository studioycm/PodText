...
- AUTHZ1-C implementation and its original closeout remain at `0147ea83947ccedb1336a71d8eecc887eb8d4e07` plus `628429236c138ad51fcb4b6d8311ad4726afb439`. The independent audit's decision-2 R-01–R-05 remediation was implemented locally at `0cdf8390b67d260ced54a0ca2ad58600bd475da5` under `prompts/pre-13-prompts/authz1c-audit-remediation-codex-prompt.md` v1. Independent remediation audit `AUTHZ1-C-REMEDIATION-AUDIT-01` is complete in `docs/research/settings-performance/18-authz1c-independent-remediation-audit.md` with decision **separate follow-up required**: a coherent keyed unowned-receipt/complete substitution can fabricate destructive rollback ownership, rollback otherwise exact-compares only selected apply-journal lineage fields, and the public source adapter aliases native integer/string IDs with the same textual value in its internal target lookup. Legacy `users.role`, ranks, Gates/macros, panel/Horizon/maintenance admission, User Resource restrictions, and every caller remain authoritative; `User` still lacks `HasRoles`. AUTHZ1-D–I remain blocked and unstarted pending resolution and independent acceptance of those three gaps. The disposable two-connection MySQL rehearsal remains a separate future gate and was not run. No production/development-database action, authority cutover, compatibility grant, management UI, AUTHZ1-D–I, ARCH1, SP3D, or MAINT-LW-UX1 work began.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Foundation/C/remediation complete locally; independent remediation audit complete with separate follow-up required; authority cutover not started | Foundation/C/audit/remediation handoffs; reports 15–18; remediation prompt v1 | R-01–R-05 are substantially supported locally, but coherent keyed ownership substitution, exact rollback apply-journal lineage, and mixed native adapter identity lookup remain open. Legacy enum/rank/Gates remain authoritative; D–I remain blocked and the MySQL rehearsal remains separate. |

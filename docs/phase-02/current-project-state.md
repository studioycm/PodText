...
- AUTHZ1-C implementation and its original closeout remain at `0147ea83947ccedb1336a71d8eecc887eb8d4e07` plus `628429236c138ad51fcb4b6d8311ad4726afb439`. The independent audit's decision-2 R-01–R-05 remediation is now implemented locally under `prompts/pre-13-prompts/authz1c-audit-remediation-codex-prompt.md` v1, with evidence in `docs/research/settings-performance/17-authz1c-audit-remediation-research.md` and `docs/phase-02/authz1c-audit-remediation-handoff.md`. V2 evidence now binds physical tuple ownership, completes ambiguous prepared/exact-planned restart as non-rollback-capable `completed_unowned`, journals truthful at-least-once cache invalidation and rollback preparation/recovery, validates the full normalized runtime schema contract, matches Laravel APP_KEY/cipher behavior, deeply validates artifacts, and refuses v1. Legacy `users.role`, ranks, Gates/macros, panel/Horizon/maintenance admission, User Resource restrictions, and every caller remain authoritative; `User` still lacks `HasRoles`. AUTHZ1-D–I remain blocked pending independent remediation review. The disposable two-connection MySQL rehearsal remains a separate future gate and was not run. No production/development-database action, authority cutover, compatibility grant, management UI, AUTHZ1-D–I, ARCH1, SP3D, or MAINT-LW-UX1 work began.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Foundation/C and audit remediation implementation complete locally; independent remediation review pending; authority cutover not started | Foundation/C/audit/remediation handoffs; `15-authz1c-analyzer-backfill-*`; `16-authz1c-independent-analyzer-backfill-audit.md`; `17-authz1c-audit-remediation-*`; remediation prompt v1 | R-01–R-05 now have v2 physical ownership, unowned recovery, at-least-once cache, rollback recovery, runtime schema, key/deep-artifact, and v1-refusal evidence. Legacy enum/rank/Gates remain authoritative; D–I remain blocked and the MySQL rehearsal remains separate. |

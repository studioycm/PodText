...
- ROLES1 user roles and multi-transcription visibility gates are implemented in the current run and awaiting the canonical implementation hash backfill: fixed enum roles, admin-panel role access, super-admin-only Users resource, the mode/role multi-transcription gate, and server-side settings save guards against forged hidden state are complete. Handoff: `docs/phase-02/roles-gates-roles1-handoff.md`; research and plan are under `docs/research/roles-gates/`.

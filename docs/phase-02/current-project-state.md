...
- AUTHZ1 is an approved prerequisite decision, not an implemented feature. Before ARCH1, PodText will replace the fixed enum/rank authorization authority with Spatie Laravel Permission plus Filament Shield, while retaining an owned immutable deployed ability catalog for stable literal dot keys, groups, bilingual labels, default role grants, and policies. BQ1/BQ2 approve multiple additive roles plus exceptional direct grants, compatibility-first initial grants, explicit `panel.admin.access`, separate `system.horizon.view`, strict delegable-role assignment, no explicit-denial/rank authority, and one transactional assignment/direct-grant audit boundary. Super-admin Gate bypass remains subject to last-super-admin/self-demotion, feature-mode, validation, conflict, reference, retention, lock, transition, and confirmation safeguards. Neither package is installed today; the existing `users.role` column, rank helpers, gates/macros, Users Resource, command, assignments, and tests remain current authority until a raw-source, fail-closed, lossless all-panel migration is accepted. Research: `docs/research/settings-performance/09-arch1-drafts-authorization-research.md` and `11-bq-decisions-and-defaults-audit.md`.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Decisions approved through BQ1/BQ2; implementation not started | `docs/research/settings-performance/09-arch1-drafts-authorization-research.md`; `11-bq-decisions-and-defaults-audit.md` | Selected Shield + Spatie Permission + deployed owned catalog, multiple additive roles, exceptional audited direct grants, strict delegated assignment, compatibility-first grants, and fail-closed all-panel migration. No dependency/schema/code change, implementation plan, or prompt exists yet. |

...
| Prompt 13 dashboard metrics | Phases 1-2 complete (of 4) | Phase 1 `7bba038` (release 74516766); phase 2 pending commit; plan `docs/phase-02/dashboard-metrics-combined-ux-plan.md`; handoffs `docs/phase-02/dashboard-metrics-phase-1-handoff.md`, `docs/phase-02/dashboard-metrics-phase-2-handoff.md` | Operator explicitly chose dashboard-first on 2026-07-31. Phase 1 ships the lens dashboard foundation: `HasFiltersForm` lens/range ToggleButtons (`DashboardLens` overview/blockers/intake, Jerusalem-aware `DashboardRange` 7/30/60 with the range hidden on the blockers lens), the cached single-source `EditorialMetrics` snapshot (funnel/blockers/structure, 60s cache, `applyReason` scoping), the context/legend widget, the blockers lens (`PublicationGapWidget` + `BlockersQueueWidget` table with reason badges, reason SelectFilter, and workspace fix action), FilamentInfoWidget removal, filawidgets theme `@source`, en+he `admin.dashboard.*` keys, and no polling anywhere (`$pollingInterval = null` on every widget). Blocked = status-published items missing a published transcription, media/embed URL, or category (incl. group inheritance); visible = full `ContentItem::published()` contract. Phase 2 builds Board 1 in order — context/legend → `PublicationFunnelWidget` (H1, per-stage micro-sparklines + gap doorway) → `EditorialStatsWidget` rebuilt as H5 composite cards (number + composition strip + filtered doorway) → `PublicationHeatmapWidget` (H4, day click) → `ActivityStreamWidget` (typed chips, listens for the heatmap day) → `LibraryCompositionWidget` (structure chips + H3 podcast health + H9 transcriber board). Adds a podcast scope filter that reaches every item-derived number (library counts stay library-wide), H6 legend chips writing to `pageFilters` through a `dashboard-filter` Livewire event, H7's burn-down bar in the queue header, stock/flow tags on every widget, `DashboardRange::dayKeys()` with PHP-side Jerusalem day bucketing, explicit `->key()` on every command-bar component, and `->queryStringIdentifier('blockersQueue')` so the queue table stops sharing the page's `filters` query keys. Phases 3 (intake lens) and 4 (RTL browser evidence) remain. |

...
- Operator/Fable accepted AUTHZ1-C planning. Audit v1 and the implementation-ready fail-closed analyzer/backfill/rollback plan are in `docs/research/settings-performance/15-authz1c-analyzer-backfill-*`; the v1 contract is `prompts/pre-13-prompts/authz1c-analyzer-backfill-codex-prompt.md`. AUTHZ1-C is planned and prompted but implementation is not started. It requires a fresh exact-version kickoff. No package assignment, cache action, authority cutover, AUTHZ1-D–I, ARCH1, or SP3D work has begun.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Foundation complete; AUTHZ1-C planned/prompted but not implemented; authority cutover not started | Foundation handoff; `docs/research/settings-performance/12-authz1-pre-implementation-research.md`; `13-authz1-foundation-research.md`; `15-authz1c-analyzer-backfill-*` | The exact reversible foundation is shipped. AUTHZ1-C audit/plan/prompt v1 freezes a raw fail-closed analyzer, accepted-fingerprint transactional role projection, post-commit cache/crash protocol, idempotency, parity, and receipt rollback. Legacy enum/rank/Gates remain authoritative; fresh implementation kickoff required. |

...
- AUTHZ1 pre-implementation research is complete, but AUTHZ1 is not implemented. `docs/research/settings-performance/12-authz1-pre-implementation-research.md` consolidates the exact five-role current-access matrix and every authorization surface; verifies a read-only Shield 4.2.0 + Spatie Permission 7.3.0 solve on Laravel 13.19/Filament 5.6.7/Livewire 4.3.3/PHP 8.4; defines the owned literal Ability catalog with HE/EN metadata, protected/reserved/delegable roles, compatibility-first grants, multiple roles, exceptional audited direct grants, strict delegated assignment, explicit `panel.admin.access`, separate `system.horizon.view`, and `public.maintenance.bypass`; and fixes the fail-closed expand/backfill/verify/cutover/rollback/contraction, MySQL final-Super-Admin concurrency, cache/worker, production, ARCH1, and SP3D acceptance boundaries. Super-admin Gate interception remains outside feature-mode, validation, conflict, reference, retention, lock, transition, confirmation, self-change, and final-Super-Admin safeguards. Neither package is installed; `users.role`, ranks, current Gates/macros, Users Resource, command, assignments, and tests remain authoritative. The single continuation point is operator/Fable acceptance of AUTHZ1-A's exact dependencies, catalog/metadata, compatibility grants, and surface disposition. No implementation plan or prompt exists.
...
| AUTHZ1 - Dynamic roles, grouped mini-abilities, and policy migration | Pre-implementation research complete; implementation not started | `docs/research/settings-performance/09-arch1-drafts-authorization-research.md`; `11-bq-decisions-and-defaults-audit.md`; `12-authz1-pre-implementation-research.md` | Exact package solve, five-role/surface matrix, catalog/role metadata/default grants, direct-grant audit, strict assignment, concurrency, cutover/rollback, cache/workers, production acceptance, and ARCH1/SP3D risks are consolidated. Next is operator/Fable acceptance of AUTHZ1-A; no dependency/schema/code change, implementation plan, or prompt exists. |

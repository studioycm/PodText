...
| SP3C - Template library and one-template editor | Prompt v3 finalized; implementation not started | `prompts/pre-13-prompts/settings-sp3c-codex-prompt.md`; historical v1 audit: `docs/research/settings-performance/06-sp3c-prompt-review.md` | V3 is the sole execution contract and incorporates the v1 review plus the second-order persistence, authorization, routing, lifecycle, reference, corruption, measurement, and compatibility corrections. No SP3C application code, tests, implementation plan, handoff, or execution ledger row exists yet. |

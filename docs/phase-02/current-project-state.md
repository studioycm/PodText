...
| Public Front v2 Step 10R-IP2 episode page header/info layout | Complete | this commit: `feat: rebuild public episode page header and info line` | Extends `item_page` with `show_breadcrumbs`, `podcast_identity`, and ordered `info_fields`; adds a settings migration and admin controls; rebuilds the public episode header with item/podcast image fallback, linked podcast identity, configured date labels/icons, linked category/tag/transcriber info fields, and site-wide link-audit tests. R1 page part and R11-R18 are landed. IP3 owns share/player/transcript reading controls next. |
...
- Prompt 13 dashboard metrics remains not started. Public Front v2 post-Step-10 mini-step sequencing is controlled by `docs/phase-02/public-front-v2-step10r-9f-mini-step-ledger.md`; Step 10R-IP2 is complete in this commit, and the next mini-step is Step 10R-IP3.

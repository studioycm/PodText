...
- ARCH1 is an approved forward architecture decision, not an implemented feature. Before SP3D, Card Templates and Public Forms must migrate from `PublicContentSettings` into independently managed, versioned model/Resource aggregates with immutable revision-owned JSON, stable identities, published pointers, exact submission revision binding, protected revision non-hydration, data-preserving cutover/rollback, and one shared after-commit configuration-change coordinator. Bounded Public Forms policy such as `require_email_verification` remains typed settings. After accepted ARCH1 migration, SP3D deletes legacy Template/Form settings branches and the surviving schema monolith, repairs/proves all three browser harnesses, and calibrates durable budgets on the final Resources and surviving settings pages. SP4 then applies slice reads/server-derived change sets to surviving singleton settings; LOG1 follows. Controlling decision and evidence record: `docs/research/settings-performance/07-sp3d-pre-research.md`.
...
| ARCH1 - Versioned Card Template and Public Form aggregates | Decisions approved; implementation not started | `docs/research/settings-performance/07-sp3d-pre-research.md` | Supersedes the settings-only forward architecture for Templates/Forms. ARCH1 research, implementation, no-loss migration, and operator acceptance precede SP3D. Current SP3C/Step 6 code remains authoritative migration-source and rollback evidence until cutover. No implementation plan or prompt has been approved. |

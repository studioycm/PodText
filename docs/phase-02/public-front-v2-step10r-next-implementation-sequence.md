...
| 8 | Step 10R-S2V | S2 | Backup visual snapshots with Playwright thumbnails/full captures, private storage, queued jobs, and gallery UI. S1 does not depend on S2V. |
| 9 | Step 10R-S1 | P1, S2 | Settings import/export package with dry-run validation, backup-before-import, transaction, and cache invalidation. |
| 10 | Step 10R-P2 | S1 | Listing fetch-window, lazy filter options/form definitions, and opt-in aggregate subselects. |
| 11 | Step 10R-P3 | P2 | Derived transcript segments and viewer render economy. |
| 12 | Step 10R-AX1 | P2, P1 | GSAP motion foundation with approved AX1-only dependency install, public-panel JS wiring, finite `PodTextMotion` preset/data-attribute contract, motion tokens, reduced-motion, and FOUC/SEO guard. |
| 13 | Step 10R-SL1 | AX1, M5 | Result display-template builder foundation, finite vocabularies, admin builder, surface selectors, grid default template, and per-template motion config. |
| 14 | Step 10R-SL2 | SL1 | Flip-slider rendering engine with scroll-snap, bounded lazy page fetch, front-face cards, RTL-aware controls, and AX1 presets. |
| 15 | Step 10R-SL3 | SL2 | Flip animation and smart side-open back face rendered from existing card templates using GSAP Flip from AX1. |
| 16 | Step 10R-SL4 | SL2 | Quick-view modal with lazy mounted content, density/label controls, full-page deep link, and AX1 open/close choreography. |
| 17 | Step 10R-AX2 | AX1 | Loading/update/page-transition concealment and motion retrofit for existing grids, sections, load-more, and page transitions. |
| 18 | Step 10R-AX3 | AX2 | Scroll-linked effects for public headers, transcript reading progress, and cover emphasis. |
| 19 | Step 10R-B4 | M1-M6 complete, IP1-IP3 complete, P1-P3 complete, SL1-SL4 complete, AX1-AX3 complete | Converge legacy card options with card presentation services, now covering slider/modal/motion surfaces. |
| 20 | Step 10R-C2 | B4 | Normalize card layout consistency and semantic layout tokens, now covering slider/modal/motion surfaces. |
| 21 | Step 9F-A | all 10R above | Rich homepage columns foundation. |
| 22 | Step 9F-B | 9F-A | Footer config and footer renderer. |
| 23 | Step 9F-C | 9F-B | Footer/rich section admin UX and integration polish. |
| 24 | Step 11 | all above and explicit Yoni approval | Seeders/demo/assets/cleanup. |
| 25 | Prompt 13 | explicit Yoni approval | Dashboard metrics. |
...
- Step 10R-S2V is inserted after S2 as a new visual-snapshot step, but S1 has no dependency on S2V and Yoni may explicitly select S1 next.
...
Continue with Step 10R-S2V by default, or Step 10R-S1 if Yoni explicitly selects the
import/export path before visual snapshots. Step 10R-UX1 standardized admin navigation, table action
...
config cache boundary. Step 10R-S2 added settings backup versions and restore. Yoni
asked this branch to continue through S2 and S1, then pause for the custom importer side
quest before returning to P2/P3/AX/SL/B4/C2/9F work; S2V is a newly inserted optional
default-next visual snapshot step. The v4 continuation order is reflected here and in
the central ledger.

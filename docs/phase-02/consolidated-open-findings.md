...
| 3.9b | **TIA's cache is machine-global and two sessions in one worktree share one graph** — collides with the S3 lane bootstrap | pest5-notes §2 | **MECHANISM CONFIRMED** `50dc246` (pestphp.com-verified), upgraded from hypothesis 2026-08-11 | **Required reading for the Phase U session.** Three facts, all doc-verified: (1) the TIA cache lives at **`~/.pest/tia/<project-key>`, keyed by normalized git remote** — machine-global, so every worktree and every concurrent session of the same repo shares **one** graph; (2) `--baseline` only **prints** the storage path — `--baselined` / `pest()->tia()->baselined()` is what actually fetches, so a briefing that says "run `--baseline`" will silently do nothing; (3) replay skips execution, but the lane infra **assumes tests execute** — `Pest.php`'s bootstrap acquires the run-lock and fingerprint, now themselves machine-global at `~/.cache/podtext-test-lane/` since S3 (`89a2ee1`/`810f6f2`). **So both TIA and the lane keep machine-global state keyed outside the worktree, and this repo is routinely worked by concurrent sessions in one tree — the exact condition neither mechanism was designed against.** Two sessions sharing a TIA graph can have one's selection shaped by the other's run. Settle this before enabling TIA, not after. Related: `test-suite-rethink-notes.md` §3 lists "where the TIA cache lives and how it behaves with two sessions sharing one worktree" as unresolved — it is now answered on the *where*, and open on the *how*. Still gated on **DP3** (no coverage driver on Herd PHP 8.4.23, measured) |
...
> **Still open: none.** A3/A4 closed at source by the ledger's dormant owner, who woke and merged them (`295f295`) — the one-owner rule held on both ends, and waiting was the right call. A11 closed on both plans (`841a1c8`, `31c536c`), A17 closed (`000e5cb`), A14 was a count note by nature. Register item 2.3 closed functionally (`a093baf`).
> **Also merged at `295f295`:** the `silent-vendor-surface` candidate the laraveldaily session registered with evidence — a routine bump shipping a surface nobody adopted.
>
...
| A4 | `service-hop-cost` (P15) | Same doc — *"open — fix queued in the post-B1 batch"* | Same doc's checklist: 105 → 3 queries; fix is `c129f5f` | ✅ **CLOSED AT SOURCE `295f295`** by the ledger's owner. ⚠️ **Method note, verified in code — do not re-flag this call site.** `BlockersQueueWidget.php:101` still calls `app(EditorialMetrics::class)->blockerReasonsFor($record)` per row, and that is **correct**: `c129f5f` moved the *cost*, not the *call*. The widget's `->query()` uses `queueQuery()`, which primes four `withExists` aliases (`has_published_transcription`, `has_published_group`, `has_group_category`, `has_own_category`, `EditorialMetrics.php:754`); `blockerFact()` (`:809`) returns the primed attribute when present and only falls back for a bare record. A query-budget test pins the flatness. **An auditor judging this site by caller shape alone re-flags a fixed defect** — same family as *linters going blind at a call site is evidence the abstraction worked* |

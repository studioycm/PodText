...
7. **Inventory line drift — and this item is its own proof.** The bridge/define block has now been cited three different ways: the reader inventory said `AppServiceProvider.php:202–207`, this doc then said `201–206`, and on 2026-08-11 it was measured at **211–216**. Every one of those was accurate when written and wrong within days. **The fix is not a fresher number.** As of 2026-08-11 the **bridge/define block and the macros** are cited by *symbol* — `Gate::policy`, `Gate::define`, `superAdminOnly`, `multiTranscription` — so grep them rather than jumping to a line. **Scope this claim honestly: the rest of this document was not de-pinned.** It still carries 34 other `file:line` citations across 24 distinct files (`CuratorMediaPolicy.php:127`, `User.php:46`, `ContentItemsTable.php:430`, the TOCTOU pairs in §5, and so on). **Two traps make that 24 hard to reproduce, so enumerate rather than count:** two files are cited sometimes by bare name and sometimes by full path, so a naive unique-string count reads 26; and one citation is a *wildcard* — `DownloadExternal*Image.php:71`, covering both `DownloadExternalContentGroupImage` and `DownloadExternalContentItemImage` — which any `[A-Za-z0-9_/.-]+\.php:` extraction breaks at, harvesting a phantom `Image.php` and still totalling 24 by coincidence. A count you can list is a count you can falsify; a count you can only cite is not., every one of them rotting on exactly the same clock as the three values above. They are left because most pin a specific statement worth landing on, not a block boundary — but treat each as a starting point to grep near, never as an address. The Sources footer is the one place line numbers are deliberately frozen: it is provenance pinned to HEAD `45a59e1`, not navigation.

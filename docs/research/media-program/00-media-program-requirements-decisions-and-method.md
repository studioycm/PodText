...
| `MI-R044` | Raster admission preserves source bytes. Positive MIME/extension, structural, size and dimension rejection remains; successful SVG sanitation is allowed for every image purpose. **Amended 2026-07-31 (RECON2 R1):** new-file admission now applies a metadata-only strip (EXIF/XMP/IPTC/comment/text segments and chunks removed at byte level, no pixel re-encode; display orientation preserved through a minimal orientation-only EXIF payload) before the recorded hash/size are taken and the file is written. Pixel data stays byte-preserved; the validator itself still returns source bytes, and the register-in-place Storage path still records existing disk bytes verbatim. | Package 3 validator fixtures; `tests/Feature/MediaAdmissionMetadataStripTest.php`. |

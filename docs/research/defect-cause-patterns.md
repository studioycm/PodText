...
## dead-guard-revival · An inert guard is woken by a fix to something else

*(Merged 2026-08-12 from the test-suite-rethink session's Pest 5 upgrade
contribution; ACTUAL, one confirmed sighting, evidence re-verified in vendor
source by the ledger owner. Name as proposed by the contributor.)*

- **Cause:** a vendor guard sits SILENTLY INERT — written, shipped, never
  reaching the thing it guards — until an unrelated correctness fix restores
  the channel it rode on. The guard then fires for the first time, possibly
  majors after it was written, and reads as a fresh regression because
  nothing in the changelog describes the behaviour that changed.
- **Evidence (ACTUAL, verified at HEAD in vendor source):**
  pest-plugin-browser's `Execution::waitForExpectation()` caps every attempt
  at a hardcoded `Playwright::usingTimeout(1_000, …)`
  (`vendor/pestphp/pest-plugin-browser/src/Execution.php:149`) — long
  predating 5.0.1. The cap never reached Playwright because the timeout rode
  only in `params`, which Playwright ≥1.62 ignores. 5.0.1 correctly began
  sending it in `metadata` as well
  (`src/Playwright/Client.php:84-89`, the reason stated in its own comment),
  which ACTIVATED the dormant cap: page scripts taking >1s were aborted
  PHP-side while their effects still landed, retried ~30×, and the loop's
  final uncapped attempt measured a page mangled by its own partial
  predecessors — proven with an in-page attempt counter (31 attempts).
  Upstream: pestphp/pest#1852. Local resolution: evidence scripts moved to
  `page()->evaluate()`, never `script()` (`5eaa7bb`, Phase U).
- **Distinct from `silent-vendor-surface`** (contributor's argument,
  accepted): that pattern is capability **gained** unnoticed — a bump brings
  something new into the tree. This is capability **restored** unnoticed —
  the code was always there, dead, and a fix to a *different* mechanism woke
  it.
- **Where else / detection:** when a bump's changelog says "X now reaches
  Y", ask **what else was riding on X being ignored**. Also: any vendor
  guard whose effect you have never actually observed firing (a cap, a
  timeout, a validation) is a candidate — absence of symptoms is not
  evidence the guard is wired.
- **Status:** ACTUAL, one sighting, resolved locally; upstream issue filed.

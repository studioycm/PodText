...
function curatorG1SafeFetcher(
    ...
    $transport->shouldReceive('get')
        ->once()
        ->with('https://cdn.example.test/image.png', 'cdn.example.test', $addressesBefore[0])
        ->andReturn($response);

    return new SafeExternalImageFetcher($dns, $transport);
}
...
it('rejects noncanonical schemes credentials fragments ports whitespace and backslashes before transport', function (string $url): void {
    ...
    expect(fn () => (new SafeExternalImageFetcher($dns, $transport))->fetch($url))
...
it('rejects private special-use mixed and rebinding DNS answers', function (): void {
    foreach ([
        ...
        expect(fn () => (new SafeExternalImageFetcher($dns, $transport))->fetch('https://cdn.example.test/image.png'))
            ->toThrow(InvalidArgumentException::class);
    }
...
it('revalidates each redirect endpoint and enforces the redirect limit', function (): void {
    ...
    $transport->shouldReceive('get')
        ->once()
        ->with('https://cdn.example.test/image.png', 'cdn.example.test', '93.184.216.34')
        ->andReturn(curatorG1ExternalResponse('', 302, ['Location' => 'https://images.example.test/final.png']));
    $transport->shouldReceive('get')
        ->once()
        ->with('https://images.example.test/final.png', 'images.example.test', '93.184.216.35')
        ->andReturn(curatorG1ExternalResponse(curatorG1ExternalPng()));

    expect((new SafeExternalImageFetcher($dns, $transport))->fetch('https://cdn.example.test/image.png'))
    ...
    expect(fn () => (new SafeExternalImageFetcher($unsafeDns, $unsafeTransport))->fetch('https://cdn.example.test/image.png'))
        ->toThrow(InvalidArgumentException::class);
});
...
it('turns validation failures into notifications while transient failures remain retryable', function (): void {
    ...
    $invalidFetcher->shouldReceive('fetch')->once()->andThrow(new InvalidArgumentException('invalid remote image'));

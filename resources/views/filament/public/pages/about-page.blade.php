    ...
    <div class="space-y-10" dir="{{ __('public.meta.dir') }}" data-test="about-page">
        <header class="space-y-3">
            @if (filled($aboutPage['kicker'] ?? null))
                <p class="text-primary-600 dark:text-primary-400 text-sm font-medium" data-test="about-kicker">
                    {{ $aboutPage['kicker'] }}
                </p>
            @endif
        ...
        <div class="space-y-8">
                ...
                @switch ($block['type'])
                    ...
                    @case ('callout')
                        ...
                        >
                            ...
                            @if (filled($block['html'] ?? null))
                                <div
                                    class="{{ $renderer->publicContentClasses() }}"
                                    data-test="about-content-typography"
                                >
                                    {!! $block['html'] !!}
                                </div>
                            @endif
                    ...
                    @case ('form_cta')
                        @if ($renderer->hasEnabledForm($block['form_key'] ?? null))
                            <section
                                class="space-y-4 rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900"
                                data-test="about-block"
                                data-block-type="form_cta"
                                data-block-key="{{ $block['key'] }}"
                            >
                                @if (filled($block['heading'] ?? null))
                                    <h2 class="text-xl font-semibold tracking-normal text-gray-950 dark:text-white">
            ...
            @if (! $teamRendered && $teamProfiles !== [])
                <x-public.about.team-section :profiles="$teamProfiles" :settings="$settings" />
            @endif
        </div>

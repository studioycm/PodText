    ...
    <x-filament::section>
        ...
        <div class="mt-3 flex flex-wrap items-center gap-2 text-xs font-medium">
            ...
            @foreach ($chipKinds as $chipKind)
                ...
                >
                    {{ $chipKind->getLabel() }} {{ \App\Support\UiFormats::number($chipKind === \App\Enums\StreamEventType::Submission ? $counts['submissions'] : $counts['imports']) }}
                </button>
            @endforeach
        </div>
        ...
        @else
                    ...
                    <li class="flex flex-wrap items-center gap-2 py-2 text-sm" data-testid="intake-row">
                        ...
                        @if ($row['subtitle'])
                            <span
                                ...
                                title="{{ __('admin.dashboard.intake.download_hint') }}"
                            >
                                {{ $row['subtitle'] }}
                            </span>
                        @endif
            ...
            @if ($counts['all'] > count($rows))
                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400" data-testid="intake-cap-note">
                    ...
                    >{{ __('admin.dashboard.intake.view_new_submissions') }}</a>
                    ·
                    <a
                        href="{{ $importsUrl }}"
                        class="hover:underline"
                    >{{ __('admin.dashboard.intake.view_imports') }}</a>
                </p>
            @endif
        @endif
    </x-filament::section>

<div class="space-y-6" dir="{{ app()->getLocale() === 'he' ? 'rtl' : 'ltr' }}">
        ...
        <x-filament::section>
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <form wire:submit="loadBackupPackage" class="space-y-4">
                    ...
                    <select
                        wire:model="selectedBackupId"
                        class="fi-select-input block w-full rounded-lg border-gray-300 bg-white text-sm text-gray-950 shadow-sm dark:border-white/10 dark:bg-gray-900 dark:text-white"
                    >
    ...
    @if ($importErrors !== [])
        <x-filament::section>
            ...
            <ul class="text-danger-600 dark:text-danger-400 list-disc space-y-1 ps-5 text-sm">
    ...
    @if ($warnings !== [])
        <x-filament::section>
            ...
            <ul class="text-warning-700 dark:text-warning-300 list-disc space-y-1 ps-5 text-sm">
    ...
    @if ($step === 'dry-run')
        <x-filament::section>
            ...
            <x-slot name="description">
                {{ __('admin.settings_import.dry_run_description', ['source' => $sourceLabel]) }}
            </x-slot>
            ...
            <div class="mb-5 flex flex-wrap items-center gap-2" data-test="settings-import-dry-run-summary">
                ...
                <x-filament::badge
                    :color="$dryRunSummary['locked'] > 0 ? 'warning' : 'gray'"
            ...
            @if ($lockedExcludedRows !== [])
                <details
                    class="border-warning-200 bg-warning-50 text-warning-900 dark:border-warning-500/30 dark:bg-warning-500/10 dark:text-warning-100 mb-5 rounded-lg border p-4 text-sm"
                    data-test="settings-import-locked-excluded"
                >
    ...
    @if ($step === 'complete')
        <x-filament::section>
            ...
            <div class="mt-3 text-sm text-gray-700 dark:text-gray-200">
                <a
                    href="{{ \App\Filament\Resources\SettingsBackups\SettingsBackupResource::getUrl() }}"
                ...
                <div class="grid gap-3 text-sm md:grid-cols-3">
                    <div>
                        <div class="font-medium text-gray-950 dark:text-white">
                            {{ __('admin.settings_import.report.mode') }}
                        </div>
                        <div class="text-gray-600 dark:text-gray-300">
                            {{ __('admin.settings_import.modes.' . $structuredImportReport->mode) }}
                        </div>
                    </div>
                    <div>
                        <div class="font-medium text-gray-950 dark:text-white">
                            {{ __('admin.settings_import.report.source') }}
                        </div>
                        <div class="text-gray-600 dark:text-gray-300">
                            {{ $structuredImportReport->sourceLabel ?? __('admin.placeholders.empty') }}
                        </div>
                    </div>
                    <div>
                        <div class="font-medium text-gray-950 dark:text-white">
                            {{ __('admin.settings_import.report.generated_at') }}
                        </div>
                        <div class="text-gray-600 dark:text-gray-300">
                            {{ $structuredImportReport->generatedAtLabel() }}
                        </div>
                    </div>
                </div>
                ...
                @if ($structuredImportReport->warnings !== [])
                    <div>
                        <h3 class="text-sm font-semibold text-gray-950 dark:text-white">
                            {{ __('admin.settings_import.report.warnings') }}
                        </h3>
                        <ul class="text-warning-700 dark:text-warning-300 mt-2 list-disc space-y-1 ps-5 text-sm">

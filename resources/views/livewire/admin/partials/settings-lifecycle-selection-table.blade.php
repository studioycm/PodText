...
<div class="space-y-4" data-test="settings-lifecycle-selection-table" data-table-mode="{{ $tableMode }}">
    ...
    @forelse ($groupedRows as $group => $groupData)
        <div
            ...
            wire:key="settings-import-group-{{ $group }}"
        >
            <div class="flex flex-wrap items-center justify-between gap-3 bg-gray-50 px-4 py-3 dark:bg-white/5">
                ...
                <x-filament::button
                    ...
                    size="xs"
                    color="gray"
                    data-test="settings-import-group-toggle"
                >
                ...
                <table class="w-full table-fixed divide-y divide-gray-200 text-start text-sm dark:divide-white/10">
                    <thead class="bg-white dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="w-56 px-4 py-3 font-semibold text-gray-950 dark:text-white">
                                {{ __('admin.fields.setting_unit') }}
                            </th>
                            <th class="px-4 py-3 font-semibold text-gray-950 dark:text-white">
                                {{ __('admin.fields.current_value') }}
                            </th>
                            <th class="px-4 py-3 font-semibold text-gray-950 dark:text-white">
                                {{ __('admin.fields.imported_value') }}
                            </th>
                            <th class="w-40 px-4 py-3 font-semibold text-gray-950 dark:text-white">
                                {{ __('admin.fields.outcome') }}
                            </th>
                        </tr>
                    </thead>
                            ...
                            >
                                <td class="px-4 py-3 align-top">
                                    <input
                                        ...
                                        class="fi-checkbox-input text-primary-600 rounded border-gray-300 disabled:opacity-40 dark:border-white/10"
                                    />
                                </td>
                                <td class="px-4 py-3 align-top">
                                    <div class="font-medium text-gray-950 dark:text-white">{{ $row['label'] }}</div>
                                    <div class="text-xs break-all text-gray-500 dark:text-gray-400">
                                        {{ $row['path'] }}
                                    </div>
                                    @if (($row['locked'] ?? false) && $tableMode !== 'locks')
                                        <div class="text-warning-700 dark:text-warning-300 mt-1 inline-flex items-center gap-1 text-xs">
                                            <x-filament::icon icon="heroicon-o-lock-closed" class="h-3.5 w-3.5" />
                                            <span>{{ __('admin.settings_import.locked_row') }}</span>
                                        </div>
                                    @endif
                                    @if ($row['error'])
                                        <div class="text-danger-600 dark:text-danger-400 mt-1 text-xs">
                                            {{ $row['error'] }}
                                        </div>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 align-top">
                                    @if ($tableMode === 'locks')
                                        <x-filament::badge
                                            :color="in_array($row['path'], $selectedPaths, true) ? 'warning' : 'gray'"
                                            :icon="in_array($row['path'], $selectedPaths, true) ? 'heroicon-o-lock-closed' : 'heroicon-o-lock-open'"
                                        >
                                            {{ in_array($row['path'], $selectedPaths, true) ? __('admin.settings_import_locks.locked') : __('admin.settings_import_locks.unlocked') }}
                                        </x-filament::badge>
                                    @else
                                        <x-filament::badge
                                            :color="$row['outcome'] === 'error' ? 'danger' : (str_starts_with($row['outcome'], 'skip') ? 'gray' : 'info')"
                                            :icon="$row['outcome'] === 'skip_locked' ? 'heroicon-o-lock-closed' : null"
                                        >

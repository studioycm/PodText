<div class="space-y-6" data-test="content-item-search">
    @if ($this->shouldRenderDiscoveryChrome())
        <div class="space-y-4" data-test="discovery-chrome">
            <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                    ...
                    <div class="grid gap-3 sm:grid-cols-[auto_auto_auto] sm:items-end">
                        ...
                        <button
                            ...
                            class="border-primary-200 bg-primary-50 text-primary-800 hover:bg-primary-100 focus:ring-primary-500 dark:border-primary-800 dark:bg-primary-950 dark:text-primary-100 dark:hover:bg-primary-900 inline-flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-sm font-medium transition focus:ring-2 focus:outline-none"
                        >
                            <span>{{ __('public.filters.open_filters') }}</span>
                            @if ($this->activeFilterCount() > 0)
                                <span
                                    class="bg-primary-700 dark:bg-primary-300 dark:text-primary-950 rounded-full px-2 py-0.5 text-xs text-white"
                ...
                <div class="mt-4 flex flex-col gap-1 text-sm text-gray-600 sm:flex-row sm:items-center sm:justify-between dark:text-gray-300">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('public.results.public_items_only') }}</p>
                </div>
            </div>
                ...
                <section class="space-y-4" data-test="homepage-section" data-section-type="{{ $sectionType }}">
                    ...
                    >
                        <div class="space-y-1">
                            ...
                            @if ($section->heading)
                                <h2
                                    ...
                                    data-test="homepage-section-heading"
                                >
                                    {{ $section->heading }}
                                </h2>
                            @endif
                        ...
                        <div
                            ...
                            data-test="homepage-section-actions"
                        >
                            @if ($isLatestSection)
                                <label
                                    class="grid gap-1 text-sm text-gray-700 dark:text-gray-200"
                                    data-test="latest-controls"
                                >
                                ...
                                @if (in_array($latestMode, ['simple', 'next_previous'], true))
                                    <div class="flex items-center gap-2" data-test="latest-next-previous">
                                        ...
                                        <span
                                            ...
                                            data-test="latest-page-indicator"
                                        >
                            ...
                            @if ($section->viewMoreUrl)
                                <a
                                    ...
                                    class="border-primary-200 bg-primary-50 text-primary-800 hover:bg-primary-100 focus:ring-primary-500 dark:border-primary-800 dark:bg-primary-950 dark:text-primary-100 dark:hover:bg-primary-900 inline-flex items-center justify-center rounded-md border px-3 py-2 text-sm font-medium transition focus:ring-2 focus:outline-none"
                    ...
                    @if ($section->sourceType === \App\Support\PublicFront\Sections\PublicDisplaySectionRegistry::CONTENT_BLOCK)
                        ...
                        >
                            ...
                            @if ($buttonLabel && $buttonUrl)
                                <a
                                    href="{{ $buttonUrl }}"
                                    class="border-primary-700 bg-primary-700 hover:bg-primary-800 focus:ring-primary-500 inline-flex items-center justify-center rounded-md border px-4 py-2 text-sm font-medium text-white transition focus:ring-2 focus:outline-none"
                        ...
                        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" data-test="categories-grid">
                            @foreach ($section->categories as $category)
                                <a
                                    ...
                                    class="hover:border-primary-300 dark:hover:border-primary-700 block rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-md dark:border-gray-800 dark:bg-gray-900"
    ...
    @elseif ($results)
        @if ($results->isNotEmpty())
            ...
            @if ($results->hasPages())
                <div data-test="content-item-pagination">{{ $results->links() }}</div>
            @endif
        @else
            <div
                class="rounded-lg border border-dashed border-gray-300 bg-white p-6 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"

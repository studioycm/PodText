...
>
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0 space-y-3">
            <h2 id="item-transcript-heading" class="sr-only">{{ __('public.pages.item.transcript_heading') }}</h2>

            @if ($activeTranscription && $details)
                <dl
                    class="flex flex-wrap items-center gap-2 text-xs text-gray-600 dark:text-gray-300"
                    data-test="transcript-details-row"
                >
                    @if ($details['title'])
                        <div
                            class="max-w-full rounded-md bg-gray-100 px-2 py-1 font-medium text-gray-800 dark:bg-gray-800 dark:text-gray-100"
                            data-test="transcript-detail-title"
                        >
                    ...
                    <div
                        ...
                        data-transcript-detail="reading-time"
                    >
                    ...
                    <div
                        class="rounded-md bg-gray-100 px-2 py-1 dark:bg-gray-800"
                        data-test="transcript-length"
                        data-transcript-detail="word-count"
                    >
                    ...
                    @if ($details['published_at'] && $details['published_part'])
                        <div
                            class="{{ $details['published_class'] }}"
                            data-test="transcript-published-at"
                            data-transcript-detail="published-at"
                        >
                    ...
                    @if ($details['transcribers'] !== [])
                        <div
                            class="rounded-md bg-gray-100 px-2 py-1 dark:bg-gray-800"
                            data-test="transcriber-link"
                            data-transcript-detail="transcribers"
                        >
                            ...
                            <dd class="flex flex-wrap gap-1">
                                @foreach ($details['transcribers'] as $transcriber)
                                    <a
                                        ...
                                        class="text-primary-700 hover:text-primary-900 dark:text-primary-300 dark:hover:text-primary-100 font-medium"
                                    >
                                        {{ $transcriber['label'] }}
                                    </a>
                                    @if (! $loop->last)
            ...
            >
                ...
                >
                    <button
                        ...
                        class="hover:text-primary-700 focus-visible:outline-primary-600 dark:hover:text-primary-200 flex w-full items-center rounded-md px-3 py-2 text-start font-medium text-gray-700 hover:bg-gray-50 focus-visible:outline-2 focus-visible:outline-offset-2 dark:text-gray-200 dark:hover:bg-gray-800"
    ...
    @if ($transcriptions->count() > 1)
        <div
            ...
            data-test="transcript-tabs"
            aria-label="{{ __('public.labels.transcriptions') }}"
        >
                ...
                >
                    ...
                    @if ($transcription->authors->isNotEmpty())
                        <span
                            ...
                            data-test="transcript-tab-transcribers"
                        >
    ...
    @if ($activeTranscription && $segments !== [])
        <div class="space-y-5">
            @foreach ($segments as $segment)
                <article
                    id="{{ $segment['anchor'] }}"
                    class="border-primary-300 scroll-mt-24 border-s-2 ps-4"
                    data-test="transcript-segment"
                >
                    <div class="mb-2 flex flex-wrap items-center gap-3 text-sm">
                        <a
                            href="#{{ $segment['anchor'] }}"
                            class="text-primary-700 hover:text-primary-900 dark:text-primary-300 dark:hover:text-primary-100 font-mono"
                            dir="ltr"

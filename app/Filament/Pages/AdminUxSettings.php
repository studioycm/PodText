                ...
                Section::make(__('admin.sections.admin_ux'))
                    ...
                    ->schema([
                        Select::make('media_naming_strategy')
                            ...
                            ->default(MediaNamingStrategy::Slug)
                            ->native(false)
                            ->required(),
                    ]),
                Section::make(__('admin.sections.media_acquisition'))
                    ...
                    ->schema([
                        ...
                        Select::make('media_acquisition_filename_strategy')
                            ...
                            ->options(MediaAcquisitionFilenameStrategy::class)
                            ->default(MediaAcquisitionFilenameStrategy::AppGenerated)
                            ->native(false)
                            ->required(),
                    ])
                    ->columns(2),
                Section::make(__('admin.sections.episode_workspace'))
                    ...
                    ->schema([
                        Select::make('transcription_presentation_mode')
                            ...
                            ->options(TranscriptionPresentationMode::class)
                            ->default(TranscriptionPresentationMode::Collapsible)
                            ->native(false)
                            ->required(),
                        Select::make('transcription_mode')
                            ->label(__('admin.fields.transcription_mode'))
                            ->helperText(__('admin.helpers.transcription_mode'))
                            ->options(TranscriptionMode::class)
                            ->default(TranscriptionMode::Single)
                            ->live()
                            ->native(false)
                            ->required()
                            ->superAdminOnly(),
                        Toggle::make('show_episode_workspace_hint_line')
                            ...
                            ->default(true)
                            ->visible(fn (Get $get): bool => $get('transcription_mode') === TranscriptionMode::Multi),

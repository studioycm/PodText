...
class PinnedExternalImageTransport
{
    ...
    public function get(string $url, string $host, string $address): Response
    {
        if (
            ...
            throw new RuntimeException('A pinned cURL transport is required for external images.');
        }
        ...
        if (! is_string($connectedAddress)) {
            throw new RuntimeException('The external image transport did not prove its connected address.');
        }

        if (! $this->sameAddress($address, $connectedAddress)) {
            throw new RuntimeException('The external image connection did not use the pinned address.');
        }
